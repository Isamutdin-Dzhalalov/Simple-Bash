...
void parser(int argc, char **argv, opt *struct_flags, char *pattern) {
	int flag;
	while((flag = getopt_long(argc, argv, "ncove:lf:hi", 0, 0)) != -1) {
		switch(flag) {
			case 'n':
				struct_flags->n = 1;
				break;
...
void flags(regex_t patt_compiled, opt *struct_flags, Counters *counters, FILE *file, char *file_name) {
	...
	while(fgets(counters->str, size - 1, file)) {
		...
		if(counters->match && struct_flags->n) {
			printf("%d:", counters->count);
		}
		if(counters->match && !struct_flags->c && !struct_flags->l && !struct_flags->o) {
...
void if_match(opt *struct_flags, Counters* counters, char *file_name) {
	if(!struct_flags->c && !struct_flags->l) {
		if(counters->file_count > 1 && !struct_flags->l && !struct_flags->h && !struct_flags->c) {
			printf("%s:", file_name);
		}
	}
}
